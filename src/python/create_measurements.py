...
BASE_FOLDER = '.data/'
CSV_FILE_NAME = 'weather_stations.csv'
TARGET_FILE_NAME = 'measurements'
...
def get_origin_file_path():
    return BASE_FOLDER + CSV_FILE_NAME
    
    
def get_target_file_path(num_rows_to_create):
    return f"{BASE_FOLDER}{TARGET_FILE_NAME}.{num_rows_to_create:_}{TARGET_FILE_EXTENSION}"    
...
def build_weather_station_name_list():
    ...
    """
    station_names = []
    base_folder = get_origin_file_path()
    with open(base_folder, 'r', encoding="utf-8", errors="ignore") as file:
...
def build_test_data(weather_station_names, num_rows_to_create):
    ...
    target_file_path = get_target_file_path(num_rows_to_create)
    ...
    print("Test data successfully written to " + target_file_path)
